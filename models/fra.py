...
class ObjectNeck(nn.Module):
    def __init__(self, 
                 ...
                 mask_type="group",
                 num_proto=64,
                 temp=1.1,  # Changed temperature to 1.1
                 **kwargs):
        ...
        self.temp = temp  # Using 1.1 as temperature
        self.eps = 1e-7
    ...
    def forward(self, x, isTrain=True):
        ...
        if self.mask_type == "attn":
            ...
            mask_embed = attn_out["mask_embed"]
            ...
            # Temperature-scaled softmax with T=1.1
            obj_attn = (dots / 1.1).softmax(dim=1) + self.eps
            ...
            obj_attn /= 1.1  # Temperature scaling with T=1.1
            obj_attn = obj_attn.view(b, self.num_heads, -1, h * w)

        if self.mask_type == "group":
            obj_attn = (obj_attn / 1.1).softmax(dim=-1)  # Temperature-scaled softmax with T=1.1
            x = x.view(b, self.num_heads, -1, h*w)
            obj_val = torch.matmul(x, obj_attn.transpose(3, 2))
            obj_val = obj_val.view(b, c, obj_attn.shape[-2])
        elif self.mask_type == "max":
            obj_attn, _ = torch.max(obj_attn, dim=1)
            out["obj_attn"] = obj_attn
            obj_attn = (obj_attn / 1.1).softmax(dim=-1)  # Temperature-scaled softmax with T=1.1
            obj_val = torch.matmul(x, obj_attn.transpose(2, 1))
        elif self.mask_type == "attn":
            obj_val = slots.transpose(2, 1)
        ...
        return z_g, obj_val, out

    def extra_repr(self) -> str:
        parts = []
        for name in ["scale", "l2_norm", "num_heads", "mask_type", "num_proto", "temp"]:
...
class FRAB_EMAN(nn.Module):
    def __init__(self, base_encoder, dim=256, m=0.996, hid_dim=4096, norm_layer=None, num_neck_mlp=2,
        ...
        self.diversity_lambda = 0.001  # Hyperparameter for diversity term
    ...
    def compute_diversity_score(self, obj_attn, h, w):
        """Compute the global diversity score δ as entropy of average assignment distribution."""
        # Average assignment probability for each region across the image: M̄(m) = (1/HW) ∑∑ M(m,u,v)
        avg_assignment = torch.sum(obj_attn, dim=(2, 3)) / (h * w)  # Shape: [batch, num_proto]
        # Compute entropy of the average assignment distribution: δ = -∑ M̄(m) log M̄(m)
        diversity_score = -torch.sum(avg_assignment * torch.log(avg_assignment + 1e-10), dim=1)  # Shape: [batch]
        return diversity_score.mean()  # Average over batch
    ...
    def loss_func(self, online, target):
        ...
        h, w = res_o["feat"].shape[2], res_o["feat"].shape[3]
        diversity_score = self.compute_diversity_score(res_o["obj_attn"], h, w)

        return loss_base, loss_inst, loss_obj, diversity_score
    ...
    def forward(self, im_v1, im_v2=None, **kwargs):
        ...
        # loss
        loss_base1, loss_inst1, loss_obj1, diversity_score1 = self.loss_func(proj_online_v1, proj_target_v2)
        loss_base2, loss_inst2, loss_obj2, diversity_score2 = self.loss_func(proj_online_v2, proj_target_v1)
        loss_base = loss_base1 + loss_base2
        avg_diversity_score = (diversity_score1 + diversity_score2) / 2.0

        loss_cluster, score = self.assign_loss(proj_online_v1, proj_online_v2, proj_target_v1, proj_target_v2)
        loss = loss_base + loss_cluster * self.loss_w_cluster + self.diversity_lambda * avg_diversity_score  # Changed from - to +
        ...
        loss_pack["div"] = self.diversity_lambda * avg_diversity_score  # Changed from negative to positive

        return loss, loss_pack

    def extra_repr(self) -> str:
        parts = []
        for name in ["loss_weight", "mask_type", "num_proto", "teacher_temp", "loss_w_obj", "loss_w_cluster", "diversity_lambda"]:
            parts.append(f"{name}={getattr(self, name)}")
        return ", ".join(parts)
