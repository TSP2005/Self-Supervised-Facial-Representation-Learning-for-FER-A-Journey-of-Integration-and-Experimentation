...
# Some code adapted from https://github.com/pytorch/examples, licensed under BSD 3-Clause License.
...
from utils.dist_utils import all_reduce_mean  # Added missing import
...
parser.add_argument("--diversity_lambda", type=float, default=0.01, help="Diversity loss weight")
parser.add_argument('--dataset', default="vggface2", help='name of dataset', choices=['in1k', 'in100', 'im_folder', 'in1k_idx', "vggface2"])
parser.add_argument('--data-root', default="./vggface2_5gb", help='root of dataset folder')
...
parser.add_argument("--local_rank", "--local-rank", default=0, type=int, help="do not set this argument")
...
parser.add_argument('--lewel-scale', default=1., type=float, help='scale factor of attention map')
...
class CFPPairs(torch.utils.data.Dataset):
    def __init__(self, root, transform=None, target_pairs=8000):
        self.root = root
...
def main(args):
    ...
    # Set random seeds for reproducibility and enforce CUDA determinism
    ...
    val_dataset = CFPPairs(root=args.cfp_root, transform=transform_test, target_pairs=2000)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        torch.manual_seed(args.seed + epoch)  # Ensure consistent shuffling per epoch
